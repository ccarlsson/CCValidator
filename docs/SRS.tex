...
\begin{itemize}[noitemsep]
    ...
    \item Performance-focused execution (compiled delegates/regex permitted)
\end{itemize}
...
\begin{itemize}[noitemsep]
    ...
    \item AOT compatibility is not required (compiled delegates and compiled regex are permitted)
\end{itemize}
...
\subsection{Validation Result API}
\begin{itemize}[noitemsep]
    \item The system shall provide \texttt{ValidationResult} with \texttt{IsValid} and a list of failures.
    \item The system shall provide \texttt{ValidationFailure} with \texttt{PropertyName}, \texttt{ErrorMessage}, and optional metadata (error code, severity, custom state).
\end{itemize}
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{NFR-3a} Compiled regex and runtime compilation are permitted when beneficial to performance.
\end{itemize}
...
\subsection{Source-Generator Friendliness (Optional)}
\begin{itemize}[noitemsep]
    \item \textbf{NFR-8 (Optional)} Rule definition should be reasonably separable from execution.
    \item \textbf{NFR-9 (Optional)} Consider APIs for precompiled rule metadata in a future version.
\end{itemize}
...
\section{Glossary}
\begin{itemize}[noitemsep]
    \item \textbf{AOT} — Ahead-of-time compilation (not a requirement for v1.0).
